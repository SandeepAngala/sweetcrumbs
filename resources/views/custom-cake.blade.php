...
@section('title', 'Customize Your Tea Combo — Mana Ooru Mana Tea')
...
<div class="relative bg-cream py-16 sm:py-24 overflow-hidden">
    ...
    <div class="relative mx-auto max-w-7xl px-6 lg:px-8">
        <div class="mb-8">
            <x-breadcrumb :items="[['label' => 'Home', 'url' => route('home')], ['label' => 'Tea Bar', 'url' => '#']]" />
        </div>

        <x-section-heading
            title="Customize Your Tea Combo"
            subtitle="Choose your chai, snack, serving size, and optional cooler — we brew it fresh at our NH 216 lounge"
            align="center"
        />
        ...
        @if($cakeTypes->isEmpty() || $sizes->isEmpty())
            <p class="text-center text-gray-600 py-12">Tea combo options are being configured. Please <a href="{{ route('contact') }}" class="text-gold font-semibold hover:underline">contact us</a> for a custom order.</p>
        @else
        <div class="mx-auto mt-16 max-w-4xl bg-white/80 backdrop-blur-md border border-amber-100 rounded-3xl shadow-xl overflow-hidden grid grid-cols-1 lg:grid-cols-3">
            <div class="bg-coffee p-8 text-cream flex flex-col justify-between lg:col-span-1">
                <div>
                    <h3 class="text-xl font-bold tracking-tight text-gold font-playfair">Your Tea Combo</h3>
                    <p class="mt-2 text-xs text-cream/70">Live summary & estimated price</p>

                    <div class="mt-8 space-y-4 text-sm border-t border-cream/10 pt-6">
                        <div class="flex justify-between"><span class="text-cream/60">Combo style:</span><span id="preview-cake-type" class="font-bold text-cream">{{ $cakeTypes->first()->value }}</span></div>
                        <div class="flex justify-between"><span class="text-cream/60">Serving:</span><span id="preview-size" class="font-bold text-cream">{{ $defaultSize?->value }}</span></div>
                        <div class="flex justify-between"><span class="text-cream/60">Tea / coffee:</span><span id="preview-shape" class="font-bold text-cream">{{ $shapes->first()?->value ?? '—' }}</span></div>
                        <div class="flex justify-between"><span class="text-cream/60">Snack:</span><span id="preview-flavor" class="font-bold text-cream">{{ $flavors->first()?->value ?? '—' }}</span></div>
                        <div class="flex justify-between"><span class="text-cream/60">Extra drink:</span><span id="preview-filling" class="font-bold text-cream">{{ $fillings->first()?->value ?? 'None' }}</span></div>
                    </div>
                </div>

                <div class="mt-12 border-t border-cream/10 pt-6">
                    <p class="text-xs text-cream/60">Estimated total</p>
                    ...
                    <p class="mt-2 text-[10px] text-cream/50 leading-relaxed">Final price may vary with seasonal snacks and add-ons.</p>
                </div>
            </div>
            ...
            <form action="{{ route('custom-cake.store') }}" method="POST" enctype="multipart/form-data" class="p-8 sm:p-10 lg:col-span-2 space-y-8">
                ...
                <div id="step-content-1" class="space-y-6">
                    <h3 class="text-lg font-bold text-coffee font-playfair border-b border-amber-50 pb-2">Step 1: Combo style & serving</h3>

                    <div>
                        <label for="cake_type" class="block text-sm font-semibold text-coffee">Combo occasion</label>
                    ...
                    <div>
                        <label for="size" class="block text-sm font-semibold text-coffee">Serving size</label>
                    ...
                    <div>
                        <label for="shape" class="block text-sm font-semibold text-coffee">Chai or coffee choice</label>
                ...
                <div id="step-content-2" class="space-y-6 hidden">
                    <h3 class="text-lg font-bold text-coffee font-playfair border-b border-amber-50 pb-2">Step 2: Snacks & extra drink</h3>

                    @if($flavors->isNotEmpty())
                    <div>
                        <label for="flavor" class="block text-sm font-semibold text-coffee">Tea-time snack</label>
                    ...
                    <div>
                        <label for="filling" class="block text-sm font-semibold text-coffee">Optional cooler / soda</label>
                ...
                <div id="step-content-3" class="space-y-6 hidden">
                    <h3 class="text-lg font-bold text-coffee font-playfair border-b border-amber-50 pb-2">Step 3: Notes & pickup details</h3>

                    <div>
                        <label for="message_on_cake" class="block text-sm font-semibold text-coffee">Special instructions (max 30 characters)</label>
                        <input type="text" name="message_on_cake" id="message_on_cake" maxlength="30" placeholder="Less sugar, extra ginger…" class="mt-2 block w-full rounded-2xl border-0 px-4 py-3 text-gray-900 shadow-sm ring-1 ring-inset ring-amber-100 bg-cream/30 focus:ring-2 focus:ring-gold text-sm" />
                    </div>

                    <div>
                        <label for="decoration" class="block text-sm font-semibold text-coffee">Combo notes</label>
                        <textarea name="decoration" id="decoration" rows="3" placeholder="Pickup time, spice level, allergies…" class="mt-2 block w-full rounded-2xl border-0 px-4 py-3 text-gray-900 shadow-sm ring-1 ring-inset ring-amber-100 bg-cream/30 focus:ring-2 focus:ring-gold text-sm"></textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-4">
                        ...
                        <div>
                            <label for="delivery_date" class="block text-sm font-semibold text-coffee">Preferred pickup date</label>
                            <input type="date" name="delivery_date" id="delivery_date" required min="{{ \Carbon\Carbon::now()->addDay()->format('Y-m-d') }}" class="mt-2 block w-full rounded-2xl border-0 px-4 py-3 text-gray-900 shadow-sm ring-1 ring-inset ring-amber-100 bg-cream/30 focus:ring-2 focus:ring-gold text-sm" />
                        </div>
                        <div>
                            <label for="budget" class="block text-sm font-semibold text-coffee">Budget (optional)</label>
                    ...
                    <div>
                        <label for="images" class="block text-sm font-semibold text-coffee">Reference photo (optional)</label>
                    ...
                    <div class="flex justify-between pt-4">
                        ...
                        <button type="submit" class="rounded-xl bg-gold px-6 py-3 text-xs font-semibold text-cream shadow-sm hover:bg-coffee transition-colors duration-300">Submit combo request &nbsp;<i class="fa-solid fa-mug-hot"></i></button>
                    </div>
                </div>
            </form>
        </div>
